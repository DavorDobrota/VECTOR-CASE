...
#ifdef USE_AVX
double calculate_mutual_inductance_double_fast_avx(
        const double N_1,
        const double L_1,
        const double R_1,
        const double r_1,
    ...
    for (int l = 0; l < 8; ++l) {
        ...
        __m256d R_1_sub_r_1 = _mm256_set1_pd(R_1_sub_r_1_arr[l]);
        ...
        for (int k = 0; k < 8; ++k) {
            ...
            for (int n = 0; n < 16; n += 4) {
                ...
                __m256d R_2_sub_r_2 = _mm256_set1_pd(R_2_sub_r_2_arr[k]);

                __m256d element = _mm256_mul_pd(R_1_sub_r_1, R_2_sub_r_2);

                __m256d vec_denom_1 = _mm256_loadu_pd(&inner_denom_1_arr[n]);
                __m256d vec_denom_2 = _mm256_loadu_pd(&inner_denom_2_arr[n]);

                loop_denom_1 = _mm256_mul_pd(loop_denom_1, vec_denom_1);
                loop_denom_2 = _mm256_mul_pd(loop_denom_2, vec_denom_2);

                __m256d L_1_plus_Z_sub_Z = _mm256_loadu_pd(&L_1_plus_Z_sub_Z_arr[n]);

                element = _mm256_mul_pd(element, L_1_plus_Z_sub_Z);
                element = _mm256_mul_pd(element, _mm256_sub_pd(loop_denom_1, loop_denom_2));

                __m256d lookup = _mm256_loadu_pd(&near_table_double_fast[k][l][n]);
                element = _mm256_mul_pd(element, lookup);

                M_12_vect = _mm256_add_pd(M_12_vect, element);
            }
        }
    }

    double M_12 = M_12_vect[0] + M_12_vect[1] + M_12_vect[2] + M_12_vect[3];
...
#ifdef USE_AVX512
double calculate_mutual_inductance_double_fast_avx512(
        const double N_1,
        const double L_1,
        const double R_1,
        const double r_1,
    ...
    double denom_1_arr[8];
    double denom_2_arr[8];
    double inner_denom_1_arr[8];
    double inner_denom_2_arr[8];
    ...
    __m512d M_12_vect = _mm512_set1_pd(0.0);

    __m512d vec_denom_1 = _mm512_load_pd(&inner_denom_1_arr[0]);
    __m512d vec_denom_2 = _mm512_load_pd(&inner_denom_2_arr[0]);

    for (int l = 0; l < 8; ++l) {

        __m512d R_1_sub_r_1 = _mm512_set1_pd(R_1_sub_r_1_arr[l]);
        ...
        for (int k = 0; k < 8; ++k) {
            ...
            __m512d R_2_sub_r_2 = _mm512_set1_pd(R_2_sub_r_2_arr[k]);

            __m512d element = _mm512_mul_pd(R_1_sub_r_1, R_2_sub_r_2);

            loop_denom_1 = _mm512_mul_pd(loop_denom_1, vec_denom_1);
            loop_denom_2 = _mm512_mul_pd(loop_denom_2, vec_denom_2);

            __m512d L_1_plus_Z_sub_Z = _mm512_load_pd(&L_1_plus_Z_sub_Z_arr[0]);

            element = _mm512_mul_pd(element, L_1_plus_Z_sub_Z);
            element = _mm512_mul_pd(element, _mm512_sub_pd(loop_denom_1, loop_denom_2));

            __m512d lookup = _mm512_load_pd(&near_table_double_fast[k][l][0]);
            element = _mm512_mul_pd(element, lookup);

            M_12_vect = _mm512_add_pd(M_12_vect, element);
        }
    }

    double M_12 = _mm512_reduce_add_pd(M_12_vect);
